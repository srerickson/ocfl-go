...
type ExecuteActivityInput struct {
	ObjectPath string           `json:"object_path"`
	Activity   *ocfl.Activity   `json:"activity"`
	ContentPath string          `json:"content_path"`
}
...
func UpdateObjectOrchestration(ctx *task.OrchestrationContext) (any, error) {
	...
	for i, activity := range activities {
		...
		if err := ctx.CallActivity(ExecuteActivityName,
			...
			return nil, fmt.Errorf("activity %d (%s) failed: %w", i, activity.ID, err)
		}
		...
		if result.Skipped {
			ctx.Logger().Info(fmt.Sprintf("[%d/%d] Skipped: %s - %s",
				i+1, len(activities), activity.ID, result.SkipReason))
		} else {
			ctx.Logger().Info(fmt.Sprintf("[%d/%d] Completed: %s (%d bytes)",
				i+1, len(activities), activity.ID, result.BytesWritten))
		}
	}
...
func CreatePlanActivity(ctx context.Context, input CreatePlanInput) ([]*ocfl.Activity, error) {
...
func ExecuteOCFLActivityFunc(ctx context.Context, input ExecuteActivityInput) (ocfl.ActivityResult, error) {
	...
	if input.Activity.Type == ocfl.ActivityTypeCopyContent {
	...
	return input.Activity.Execute(ctx, fsys, ".", src)
}
