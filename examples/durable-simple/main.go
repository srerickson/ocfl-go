...
func run() error {
	...
	builder := obj.NewUpdatePlanBuilder(stage).
		...
		WithMessage("Example durable update").
		WithUser(&ocfl.User{
	...
	// Step 5: Get activities from the plan
	...
	for i, activity := range activities {
		...
		if state.CompletedSteps[activity.ID] {
			fmt.Printf("   [%d/%d] ⏭️  Skipping (already done): %s\n",
				i+1, len(activities), activity.ID)
			continue
		}
		...
		if err != nil {
			...
			return fmt.Errorf("activity %q failed after 3 attempts: %w", activity.ID, err)
		}

		// Mark as completed
		state.CompletedSteps[activity.ID] = true
		state.ActivityResults[activity.ID] = ActivityInfo{
			Name:         activity.ID,
		...
		fmt.Printf("   [%d/%d] %s %s\n", i+1, len(activities), status, activity.ID)
...
func executeActivityWithRetry(
	...
	result, err := activity.Execute(ctx, fsys, ".", src)
	return result, err
}
