...
//E004: The [version declaration] filename MUST conform to the pattern T=dvalue, where T must be 0, and dvalue must be ocfl_object_, followed by the OCFL specification spec.Number.
var E004 = validation.NewCode("E004",
	map[spec.Num]*validation.Ref{
		{1,0}:{
			Description: "The [version declaration] filename MUST conform to the pattern T=dvalue, where T must be 0, and dvalue must be ocfl_object_, followed by the OCFL specification spec.Number.",
...
//E005: The [version declaration] filename must conform to the pattern T=dvalue, where T MUST be 0, and dvalue must be ocfl_object_, followed by the OCFL specification spec.Number.
...
//E006: The [version declaration] filename must conform to the pattern T=dvalue, where T must be 0, and dvalue MUST be ocfl_object_, followed by the OCFL specification spec.Number.
...
//E009: The spec.Number sequence MUST start at 1 and must be continuous without missing integers.
var E009 = validation.NewCode("E009",
	map[spec.Num]*validation.Ref{
		{1,0}:{
			Description: "The spec.Number sequence MUST start at 1 and must be continuous without missing integers.",
...
//E010: The spec.Number sequence must start at 1 and MUST be continuous without missing integers.
...
//E040: [head] must be the version directory name with the highest spec.Number.
var E040 = validation.NewCode("E040",
	map[spec.Num]*validation.Ref{
		{1,0}:{
			Description: "[head] must be the version directory name with the highest spec.Number.",
...
//E077: [The OCFL version declaration filename] MUST conform to the pattern T=dvalue, where T must be 0, and dvalue must be ocfl_, followed by the OCFL specification spec.Number.
var E077 = validation.NewCode("E077",
	map[spec.Num]*validation.Ref{
		{1,0}:{
			Description: "[The OCFL version declaration filename] MUST conform to the pattern T=dvalue, where T must be 0, and dvalue must be ocfl_, followed by the OCFL specification spec.Number.",
...
//E078: [The OCFL version declaration filename] must conform to the pattern T=dvalue, where T MUST be 0, and dvalue must be ocfl_, followed by the OCFL specification spec.Number.
...
//E079: [The OCFL version declaration filename] must conform to the pattern T=dvalue, where T must be 0, and dvalue MUST be ocfl_, followed by the OCFL specification spec.Number.
...
//E104: Version directory names MUST be constructed by prepending v to the spec.Number.
var E104 = validation.NewCode("E104",
	map[spec.Num]*validation.Ref{
		{1,1}:{
			Description: "Version directory names MUST be constructed by prepending v to the spec.Number.",
			URL: "https://ocfl.io/draft/spec/#E104",
		},	
	})

//E105: The spec.Number MUST be taken from the sequence of positive, base-ten integers: 1, 2, 3, etc.
var E105 = validation.NewCode("E105",
	map[spec.Num]*validation.Ref{
		{1,1}:{
			Description: "The spec.Number MUST be taken from the sequence of positive, base-ten integers: 1, 2, 3, etc.",
...
//W001: Implementations SHOULD use version directory names constructed without zero-padding the spec.Number, ie. v1, v2, v3, etc.
var W001 = validation.NewCode("W001",
	map[spec.Num]*validation.Ref{
		{1,0}:{
			Description: "Implementations SHOULD use version directory names constructed without zero-padding the spec.Number, ie. v1, v2, v3, etc.",
