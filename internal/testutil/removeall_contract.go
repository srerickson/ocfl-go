...
// WriteFSRemoveAllContract configures the shared [WriteFS.RemoveAll] contract
// test (TestWriteFSRemoveAllContract) with the parts of the contract
// [ocflfs.WriteFS] leaves to the backend.
type WriteFSRemoveAllContract struct {
	...
	// directory. WriteFS.RemoveAll documents this as backend-dependent: the
	...
	// (os.RemoveAll does); a backend that treats the name as a directory
	// prefix does not, because a file's key is not under its own prefix.
	RemoveAllOnFileRemovesIt bool
}

// TestWriteFSRemoveAllContract asserts the [ocflfs.WriteFS] RemoveAll behavior
// that all backends share, against whichever backend fsys implements.
//
...
// the other: RemoveAll is documented to return nil for a path that does not
// exist, which is the opposite of Remove and easy to implement backwards.
