...
// because S3 has no directories: its listing is flat, and the directory-shaped
// answer has to be reconstructed from key prefixes. Three properties keep the
// two interchangeable for callers, and each is a distinct way that
...
//   - entries are sorted, as DirEntriesFS documents, so callers may rely on
//     the order without sorting defensively.
//
...
func TestDirEntriesContract(t *testing.T, fsys ocflfs.WriteFS) {
	...
	t.Run("yields base names one level deep, sorted", func(t *testing.T) {
		...
		be.AllEqual(t, []string{"alpha.txt", "middle.txt", "sub", "zeta.txt"}, names)
		...
		// drop it.
		be.True(t, sort.StringsAreSorted(names))
	...
	t.Run("descends one level at a time", func(t *testing.T) {
		...
		be.AllEqual(t, []string{"child.txt", "deeper"}, names)
	})
