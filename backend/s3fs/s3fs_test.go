...
var (
	...
	bucket      = flag.String("bucket", "ocfl-s3-test", "bucket name")
...
func TestS3Backend(t *testing.T) {
	...
	createBucket(t, s3cli, *bucket)
	cleanBucket(t, s3cli, *bucket)
...
func TestFS(t *testing.T) {
	...
	// big directory with 1001 files
	const numfiles = 1001
	const bigdir = "bigdir"
	bigdirModes := make([]fs.FileMode, numfiles)
	bigdirNames := make([]string, numfiles)
	bigdirSizes := make([]int, numfiles)
	bigdirIsDir := make([]bool, numfiles)
	for i := 0; i < numfiles; i++ {
		name := fmt.Sprintf("file-%04d.txt", i)
		allFiles = append(allFiles, path.Join(bigdir, name))
		bigdirNames[i] = name
		bigdirModes[i] = 0
		bigdirSizes[i] = len(content)
		bigdirIsDir[i] = false
	}
	...
	t.Run("list empty bucket", func(t *testing.T) {
		fi, err := s3ocfl.New(s3cl, *bucket).Open(".")
		if err != nil {
	...
	testFn := func(t *testing.T, s3fs *s3ocfl.Backend) {
			...
			t.Run("success", func(t *testing.T) {
				fixtures := []struct {
				...
				}{
					{
						desc:  "top level",
						path:  ".",
						names: []string{"a", "a.txt", "bigdir", "dir", "dir1", "dir2", testFile, "x", "y.txt", "y2.txt", "y3.txt", "z"},
						modes: []fs.FileMode{fs.ModeDir, 0, fs.ModeDir, fs.ModeDir, fs.ModeDir, fs.ModeDir, 0, fs.ModeDir, 0, 0, 0, fs.ModeDir},
						isDir: []bool{true, false, true, true, true, true, false, true, false, false, false, true},
						size:  []int{0, len(content), 0, 0, 0, 0, len(content), 0, len(content), len(content), len(content), 0},
					},
					{
						desc:  "dir1",
						path:  "dir1",
	...
	fixtures := []struct {
		...
		{desc: "standard", s3fs: s3ocfl.New(s3cl, *bucket)},
		{desc: "max keys = 1", s3fs: s3ocfl.New(&client{MaxKeys: aws.Int64(1), S3API: s3cl}, *bucket)},
		{desc: "max keys = 2", s3fs: s3ocfl.New(&client{MaxKeys: aws.Int64(2), S3API: s3cl}, *bucket)},
		{desc: "max keys = 3", s3fs: s3ocfl.New(&client{MaxKeys: aws.Int64(3), S3API: s3cl}, *bucket)},
	}
...
func cleanBucket(t *testing.T, cl s3iface.S3API, bucket string) {
	...
	err := cl.ListObjectsV2Pages(in, eachPage)
	if err != nil {
