...
// Merge returns a new map that is the result of merging digests and paths from
// the two maps. An error is returned if either Map is invalid or if the merged
// map would be invalid (e.g., if the same paths is found in both input maps but
// with different digests).
func (m1 Map) Merge(m2 *Map) (*Map, error) {
	if m2 == nil {
	...
	maker, err := MapMakerFrom(m1)
	if err != nil {
		return nil, err
	}
	m2.EachPath(func(name, digest string) error {
		return maker.Add(digest, name)
	})
